...
# VIC-I multicolor codes: 00 background,01 border,10 character,11 auxiliary.
PALETTE=dict(background=6,border=0,wallDark=3,wallLight=11)
MC_COLOR=8|PALETTE['wallDark']
MC_AUX=PALETTE['wallLight']<<4  # VIC audio volume remains zero.
MC_BACKGROUND=(PALETTE['background']<<4)|8|PALETTE['border']
...
def strip(top,side):
    # Solid 2-bit wall colors; floor alone uses a black/blue checker.
    return bytes(0 if y<top else (0xaa if side else 255)
                 if y<96-top else (0x44 if y%2==0 else 0x11) for y in range(96))
...
def framebuffer(g,pose):
    ...
    for x,top in enumerate(tops):
        pattern=strip(top,rays[x//2][1]);mask=0xc0>>((x&3)*2)
        for y in range(96):out[(y//8)*128+(x//4)*8+(y&7)] |= pattern[y]&mask
    return out,rays
