...
def edge_tables():
    source=''
    for k in (1,3):source+=emit(f'edge_lerp_{k}',bytes(((d*k+2)//4)&255 for d in range(-48,49)))
    for plane in ('lo','hi'):
        source+=emit(f'edge_row_{plane}',bytes((((y//8)*128+(y&7))>>(8 if plane=='hi' else 0))&255 for y in range(96)))
    source+=emit('edge_masks',bytes(0xc0>>((x&3)*2) for x in range(64)))
    source+=emit('edge_columns',bytes((x//4)*8 for x in range(64)))
    source+=emit('edge_floor',bytes(0x44 if y%2==0 else 0x11 for y in range(96)))
    source+=emit('edge_shade',bytes([0xaa]*96))
    return source
...
def build(mode='interactive',scene_file=None,out=None):
    ...
    assert all(type(v)==int and 0<=v<16 for v in PALETTE.values())
    assert PALETTE['border']<8 and PALETTE['wallDark']<8
    constants+=f'MC_COLOR={MC_COLOR}\nMC_AUX={MC_AUX}\nMC_BACKGROUND={MC_BACKGROUND}\n'
    ray,fields=raycode()
