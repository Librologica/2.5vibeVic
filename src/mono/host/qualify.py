...
from model import framebuffer,INITIAL,simulation,ROUTE
...
def check(out,min_frames=120):
    ...
    for f in sorted(out.glob('frame-*-ram.bin')):
        ...
        assert all(b&15==1 for b in color),(f,'color RAM')
        assert vic[2]==0x90 and vic[3]&127==24 and vic[5]==(0xdc,0xfe)[shown] and vic[15]==8,(f,vic.hex())
